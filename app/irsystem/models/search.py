...
class SearchEngine():
    def __init__(self, should_build_structures, test=False):
    ...
    def search(self, query):
        if self.inverted_index is None:
            self.inverted_index = InvertedIndex()
            self.inverted_index.load()
        ranks = compare_string_to_posts(query, self.inverted_index, self.idf, self.norms, self.sentiment_lookup, 0.25)
